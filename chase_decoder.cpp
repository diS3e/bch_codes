//
// Created by S3 on 11.07.2023.
//
#include <map>
#include <algorithm>
#include "chase_decoder.h"

chase_decoder::chase_decoder(bch_code &bchCode1) : bchCode(bchCode1) {}
...
std::vector<int> chase_decoder::intToBinary(int number) {
...
std::vector<std::vector<int>> chase_decoder::chase_decoding_2(std::vector<double> &corrupted_word, int tau) {
...
int chase_decoder::get_even(std::vector<int> &polynomial, int degree) {
    return get(degree * 2, polynomial);
}

int chase_decoder::get_odd(std::vector<int> &polynomial, int degree) {
    return get(degree * 2 + 1, polynomial);
}


std::vector<int> chase_decoder::get_modified_syndrome(std::vector<int> &syndrome, int t) {
    std::vector<int> a(t);
...
grobner_basis chase_decoder::fitzpatrick(std::vector<int> &syndrome, int t) {
    ...
    for (int k = 0; k < t; ++k) {
        ...
        alpha[j] = get(k, bchCode.multiply_polynomial(syndrome, b[j]));
        if (alpha[i] != 0) {
            int coeff = bchCode.GF.multiply_elements(alpha[1 ^ i], bchCode.GF.getInverse(alpha[i]));
//                bchCode.scalar_mul_vector(coeff, b[i], b[i]);
...
std::vector<int> mu(std::pair<std::vector<int>, std::vector<int>> &polynomial_pair) {
    ...
    for (int i = 0; i < result.size(); ++i) {
        if ((i & 1) == 0) {
            result[i] = get(i >> 1, polynomial_pair.second);
        } else {
            result[i] = get((i - 1) >> 1, polynomial_pair.first);
        }
    }
    return result;
}
...
grobner_basis chase_decoder::kotter_iteration(double w, grobner_basis &G,
    ...
    if (hh1 != 0) {
        ...
        delta[0] = bchCode.evaluate(G.first.first, inverse_square) ^
                   GF.multiply_elements(coef, bchCode.evaluate(G.first.second,
                                                               inverse_square));
        delta[1] = bchCode.evaluate(G.second.first, inverse_square) ^
    ...
    for (int j: A) {
        ...
        if (j != J) {
            ...
            to.first = bchCode.summing_polynomial(bchCode.multiply_polynomial({coef}, G_j.first), G_J.first);
            to.second = bchCode.summing_polynomial(bchCode.multiply_polynomial({coef}, G_j.second),
                                                   G_J.second);
        } else {
            to.first = bchCode.multiply_polynomial({inverse_square, 1}, G_J.first);
            to.second = bchCode.multiply_polynomial({inverse_square, 1}, G_J.second);
        }
    }
    return Gnew;
}

void chase_decoder::dfs(std::vector<int> &tree, std::vector<std::vector<int>> &layers, int u, int tau, int depth) {
...
std::pair<std::vector<int>, std::vector<std::vector<int>>> chase_decoder::get_decoding_tree(int tau) {
...
std::vector<std::vector<int>> chase_decoder::fast_chase_decoding_2(std::vector<double> &corrupted_word, int tau) {
    //Инициализация
    auto signs = get_signs(corrupted_word);
    auto reliability = get_reliability(corrupted_word);
...
//                    int d = bchCode.evaluate(caph2, inverse);
            ...
            int delta = std::max(2 * bchCode.degree(f_1) + 2 * bchCode.degree(H.first.first) + 1,
                                 2 * bchCode.degree(f_2) + 2 * bchCode.degree(H.second.second));
...
int get_even(std::vector<int> &polynomial, int degree) {
    return bch_code::get(degree * 2, polynomial);
}

int get_odd(std::vector<int> &polynomial, int degree) {
    return bch_code::get(degree * 2 + 1, polynomial);
}
...
std::vector<int> mu(std::pair<std::vector<int>, std::vector<int>> &polynomial_pair) {
    ...
    for (int i = 0; i < result.size(); ++i) {
        if ((i & 1) == 0) {
            result[i] = bch_code::get(i >> 1, polynomial_pair.second);
        } else {
            result[i] = bch_code::get((i - 1) >> 1, polynomial_pair.first);
        }
    }
    return result;
}
...
//                    int c = bchCode.evaluate(caph1, inverse);
//                    int d = bchCode.evaluate(caph2, inverse);
                if (bchCode.GF.multiply_elements(a, evaluated_caph1[i]) == bchCode.GF.multiply_elements(b, evaluated_caph2[i])) {
                    errors.push_back(i);
                }
            }
